...
func TestEntrypointConfig(t *testing.T) {
	...
	v.Set("ep.http.read-timeout", "1s")
	v.Set("ep.http.read-header-timeout", "2s")
	v.Set("ep.http.write-timeout", "3s")
	v.Set("ep.http.idle-timeout", "4s")
	v.Set("ep.http.max-header-bytes", 1024)
	v.Set("ep.net.keep-alive", "-10s")
	v.Set("ep.net.keep-alive-probe.enable", true)
	v.Set("ep.net.keep-alive-probe.idle", "5s")
	v.Set("ep.net.keep-alive-probe.interval", "6s")
	v.Set("ep.net.keep-alive-probe.count", 3)
	v.Set("ep.tls.cert-file", "test-cert.pem")
	v.Set("ep.tls.key-file", "test-key.pem")
